...
int main(int argc, char *argv[]) {
  ...
  struct programState* states[50];
  struct queue* inputQueues[50];
  struct queue* outputQueues[50];

  for (int i = 0; i < 50; i++) {
    states[i] = initState(program, i);
    inputQueues[i] = newQueue(1, i);
  ...
  while (!halt) {
    ...
    for (int i = 0; i < 50; i++) {
      ...
      if (outputQueues[i]->length >= 3) {
        int destination = queueGet(outputQueues[i]);
        long long X = queueGet(outputQueues[i]);
        long long Y = queueGet(outputQueues[i]);
